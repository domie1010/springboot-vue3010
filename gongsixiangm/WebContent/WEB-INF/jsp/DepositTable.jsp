...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/depositAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("deposit/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="deposit/informationLoad" method="POST"
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${depositList.datas }" var="deposit" varStatus="vg">
						<tr>
							<td><c:out value="${deposit['project']['name']}"></c:out></td>
							<td><c:out value="${deposit['price']}"></c:out></td>
							<td><c:out value="${deposit['createtime']}"></c:out></td>
							<td><a href="javascript:;" onclick="update(${deposit['id']})"
								class="tablelink">查看</a> <a href="javascript:;"
								onclick="deletecheck(${deposit['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="deposit/informationLoad?pageNow=${pager.pageNow }&p_id=${deposit['p_id']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="deposit/informationLoad?pageNow=${pager.pageNow-1 }&p_id=${deposit['p_id']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="deposit/informationLoad?pageNow=${vs.count }&p_id=${deposit['p_id']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="deposit/informationLoad?pageNow=${count }&p_id=${deposit['p_id']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="deposit/informationLoad?pageNow=${count }&p_id=${deposit['p_id']}">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="deposit/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
