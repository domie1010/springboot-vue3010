...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/missionAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("mission/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="mission/informationLoad" method="POST"
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${missionList.datas }" var="mission" varStatus="vg">
						<tr>
							<td><c:out value="${mission['name']}"></c:out></td>
							<td>
								<c:if test="${mission['status'] eq 0}" var="s1">
									<c:out value="未完成" />
								</c:if>
								<c:if test="${mission['status'] eq 1}" var="s1">
									<c:out value="已完成" />
								</c:if>
							</td>
							<td><c:out value="${mission['project']['name']}"></c:out></td>
							<td><c:out value="${mission['userInfo']['userName']}"></c:out></td>
							<td><c:out value="${mission['createtime']}"></c:out></td>
							<td><a href="javascript:;" onclick="update(${mission['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="mission/informationLoad?pageNow=${pager.pageNow }&p_id=${mission['p_id']}&user_id=${mission['user_id']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="mission/informationLoad?pageNow=${pager.pageNow-1 }&p_id=${mission['p_id']}&user_id=${mission['user_id']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="mission/informationLoad?pageNow=${vs.count }&p_id=${mission['p_id']}&user_id=${mission['user_id']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="mission/informationLoad?pageNow=${count }&p_id=${mission['p_id']}&user_id=${mission['user_id']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="mission/informationLoad?pageNow=${count }&p_id=${mission['p_id']}&user_id=${mission['user_id']}">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="mission/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
