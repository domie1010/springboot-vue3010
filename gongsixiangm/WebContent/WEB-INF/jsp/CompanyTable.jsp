...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/companyAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("company/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="company/informationLoad" method="POST"
		...
		<div class="rightinfo">
			...
			<div class="tools">
				<ul class="toolbar">
					...
					<li class="click"><font color="#0099cc"><a href="company/exportExcel">表格导出</a>
					</font></li>
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${companyList.datas }" var="company" varStatus="vg">
						<tr>
							...
							<td><c:out value="${company['createtime']}"></c:out></td>
							<td><a href="javascript:;" onclick="update(${company['id']})"
								class="tablelink">查看</a> <a href="javascript:;"
								onclick="deletecheck(${company['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="company/informationLoad?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="company/informationLoad?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="company/informationLoad?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="company/informationLoad?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="company/informationLoad?pageNow=${count }">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="company/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
