...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/costAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("cost/informationLoadOne?id="+id);
	        }
...
<body>
	...
	<form action="cost/informationLoad" method="POST"
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${costList.datas }" var="cost" varStatus="vg">
						<tr>
							<td><c:out value="${cost['project']['name']}"></c:out></td>
							<td><c:out value="${cost['price']}"></c:out></td>
							<td><c:out value="${cost['createtime']}"></c:out></td>
							<td><a href="javascript:;" onclick="update(${cost['id']})"
								class="tablelink">查看</a> <a href="javascript:;"
								onclick="deletecheck(${cost['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="cost/informationLoad?pageNow=${pager.pageNow }&p_id=${cost['p_id']}">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="cost/informationLoad?pageNow=${pager.pageNow-1 }&p_id=${cost['p_id']}">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="cost/informationLoad?pageNow=${vs.count }&p_id=${cost['p_id']}">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="cost/informationLoad?pageNow=${count }&p_id=${cost['p_id']}">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="cost/informationLoad?pageNow=${count }&p_id=${cost['p_id']}">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="cost/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
