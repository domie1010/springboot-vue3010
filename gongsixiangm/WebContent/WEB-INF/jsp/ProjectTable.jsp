...
<script type="text/javascript">
        $(function(){
        	$("#add").click(function () {
                frameset_layer().open("request/projectAdd");
            });

            
	        })
	        function update(id){
	            	frameset_layer().open("project/informationLoadOne?id="+id);
	        }
        ...
        function newMission(id){
        	frameset_layer().open("request/missionAdd?id="+id);
        }
        </script>
<script type="text/javascript">
...
<body>
	...
	<form action="project/informationLoad" method="POST"
		...
		<div class="rightinfo">
			...
			<table class="tablelist">
				...
				<tbody>
					...
					<c:forEach items="${projectList.datas }" var="project" varStatus="vg">
						<tr>
						
							<td><c:out value="${project['name']}"></c:out></td>
							<td><c:out value="${project['price']}"></c:out></td>
							<td>
							<c:if test="${project['count_complete']+project['count_uncomplete'] ne 0}">
							<fmt:formatNumber type="number" value="${project['count_complete']/(project['count_uncomplete']+project['count_complete'])*100 }" pattern="0.00" maxFractionDigits="2"/>%
							</c:if>
							<c:if test="${project['count_complete']+project['count_uncomplete'] eq 0}">
							<c:out value="暂无进度" />
							</c:if>
							</td>
							<td><a href="cost/informationLoad?p_id=${project['id'] }" class="tablelink">查看</a></td>
							<td><a href="deposit/informationLoad?p_id=${project['id'] }" class="tablelink">查看</a></td>
							<td><a href="pay/informationLoad?p_id=${project['id'] }" class="tablelink">查看</a></td>
							<td>
							<a href="mission/informationLoad?p_id=${project['id'] }" class="tablelink">查看</a>
							<a href="javascript:;" onclick="newMission(${project['id']});" class="tablelink">新建进度</a>
							</td>
							<td><c:out value="${project['createtime']}"></c:out></td>
							<td><a href="javascript:;" onclick="update(${project['id']})"
								class="tablelink">查看</a> <a href="javascript:;"
								onclick="deletecheck(${project['id']})"
			...
			<div class="pagin">
				...
				<ul class="paginList">
					<li class="paginItem"><c:if test="${pager.pageNow le 1 }"
							var="pagele">
							<a
								href="project/informationLoad?pageNow=${pager.pageNow }">
								<span class="pagepre"></span>
							</a>
						</c:if> <c:if test="${!pagele }" var="pagele">
							<a
								href="project/informationLoad?pageNow=${pager.pageNow-1 }">
								<span class="pagepre"></span>
							</a>
						</c:if></li>
					...
					<c:if test="${pager.totalPages lt 10}">
						<c:forEach var="count" begin="1" end="${pager.totalPages }"
							...
							<c:if test="${vs.count eq pager.pageNow }" var="current">
								...
								href="project/informationLoad?pageNow=${vs.count }">${vs.count}</a></li>
							</c:if>
							<c:if test="${!current }">
						...
						<c:if
							...
							<c:forEach var="count" begin="${pager.totalPages-9}"
								...
								<c:if test="${count eq pager.pageNow }" var="current">
									<li class="paginItem current"><a
									href="project/informationLoad?pageNow=${count }">${count}</a></li>
								</c:if>
								<c:if test="${!current }">
									<li class="paginItem"><a
									href="project/informationLoad?pageNow=${count }">${count}</a></li>
        ...
        function deletecheck(id){
        	$(function(){
                if(window.confirm("您确定删除该条数据吗?")){
        			window.location.href="project/informationDelete?id="+id;
        		}
        	})
        }
        $(function() {
